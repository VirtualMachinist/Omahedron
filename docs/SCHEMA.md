...
`scripts` contains the upstream inventory plus the permanent `pacman` policy row. `port_scripts` separately inventories the five `omarchy-nix-*` helpers implemented in `pkgs/omarchy.nix`. Generated hidden wrapper internals are not public commands. `since_pin` records when the row was first classified. Executable bits are checked against `executable` (default true). The sourced Bash library and upstream's non-executable `omarchy-remove-service-dropbox` are explicit exceptions; the latter's menu action uses the Nix catalog handler, while direct execution remains unavailable.

ADR-0009 requires the first stub line to use `omahedron: stub: <reason>` or `omahedron: na: <reason>`. The inherited implementation still has `NixOS: …` messages and documented silent predicates/no-ops. JSON and [COMPAT](COMPAT.md) record that debt; this ledger cleanup does not change command output or pretend prefix compliance is complete.
