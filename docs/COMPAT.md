...
| Stub output contract | stub | Diagnostic stubs start with `omahedron: stub: nixos-declarative`, followed by the existing NixOS guidance. Exit codes and silent predicate/no-op behavior are preserved and tested for every stub. |
...
| Dropbox removal helper mode | vendor | Upstream `bin/omarchy-remove-service-dropbox` lacks executable bits. The package repairs them while preserving the upstream body; the menu still uses the Nix catalog handler. Its downstream package-removal call reaches the NixOS stub. |
