...
| Automatic printer discovery | host | Omahedron still defaults `services.printing.browsed.enable = true`, while upstream v4.0.2 migration `1788009111.sh` removes `cups-browsed`. The Arch removal migration and hardening drop-in are not applied; NixOS owns the service. Review the discovery default before the product tag. |
